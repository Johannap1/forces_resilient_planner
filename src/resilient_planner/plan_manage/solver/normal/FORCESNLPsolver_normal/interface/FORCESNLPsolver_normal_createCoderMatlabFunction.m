...
% 
% FORCESNLPsolver_normal_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the FORCESNLPsolver_normal FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% FORCESNLPsolver_normal_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% FORCESNLPsolver_normal_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% FORCESNLPsolver_normal_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function FORCESNLPsolver_normal_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'FORCESNLPsolver_normal_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'FORCESNLPsolver_normal_block';
    end
    ...
    function_name = 'FORCESNLPsolver_normal_coderFunction.m';
    position = [170, 99, 650, 520];
    parameter_sizes = struct('xinit', '[9,1]',...
                             'x0', '[340,1]',...
                             'all_parameters', '[2600,1]',...
                             'num_of_threads', '[1,1]');
    if useCompactBlock
        function_name = 'FORCESNLPsolver_normal_coderFunctioncompact.m';
