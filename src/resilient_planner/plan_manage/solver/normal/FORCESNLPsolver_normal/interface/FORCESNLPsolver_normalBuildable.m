...
% 
classdef FORCESNLPsolver_normalBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'FORCESNLPsolver_normal'))
                solverpath = fullfile(buildablepath, 'FORCESNLPsolver_normal');
            end
            ForcesUpdateBuildInfo(buildInfo, cfg, 'FORCESNLPsolver_normal', solverpath, 1, true);
        end
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_x12 = coder.nullcopy(zeros(17, 1));
            outputs_x13 = coder.nullcopy(zeros(17, 1));
            outputs_x14 = coder.nullcopy(zeros(17, 1));
            outputs_x15 = coder.nullcopy(zeros(17, 1));
            outputs_x16 = coder.nullcopy(zeros(17, 1));
            outputs_x17 = coder.nullcopy(zeros(17, 1));
            outputs_x18 = coder.nullcopy(zeros(17, 1));
            outputs_x19 = coder.nullcopy(zeros(17, 1));
            outputs_x20 = coder.nullcopy(zeros(17, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = FORCESNLPsolver_normalBuildable.forcesCall(params.xinit, params.x0, params.all_parameters, params.num_of_threads);
        end

        function [output,exitflag,info] = forcesCall(xinit, x0, all_parameters, num_of_threads)
            solvername = 'FORCESNLPsolver_normal';
            ...
            [output_c, exitflag_c, info_c] = FORCESNLPsolver_normalBuildable.forcesInitOutputsC();
            ...
            [output, exitflag, info] = FORCESNLPsolver_normalBuildable.forcesInitOutputsMatlab();
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_x12 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x13 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x14 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x15 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x16 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x17 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x18 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x19 = coder.nullcopy(double(zeros(17, 1)));
            outputs_x20 = coder.nullcopy(double(zeros(17, 1)));
            output = struct('x01', outputs_x01,...
                            'x02', outputs_x02,...
                            'x03', outputs_x03,...
                            'x04', outputs_x04,...
