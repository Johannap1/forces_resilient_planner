...
typedef FORCESNLPsolver_normal_float solver_float;
...
void copyCArrayToM_FORCESNLPsolver_normal(FORCESNLPsolver_normal_float *src, double *dest, solver_int32_default dim) 
{
...
void copyCValueToM_FORCESNLPsolver_normal(FORCESNLPsolver_normal_float* src, double* dest)
{
    *dest = (double)*src;
}

void copyMArrayToC_FORCESNLPsolver_normal(double *src, FORCESNLPsolver_normal_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
...
void copyMValueToC_FORCESNLPsolver_normal(double * src, FORCESNLPsolver_normal_float * dest)
{
	*dest = (FORCESNLPsolver_normal_float) *src;
}
...
void mexFunction( solver_int nlhs, mxArray *plhs[], solver_int nrhs, const mxArray *prhs[] )  
{
    ...
    arr = prhs[0];
    copyMArrayToC_FORCESNLPsolver_normal(mxGetPr(arr), z, nvar);
    if (nrhs > 1)
	{
        arr = prhs[1];
        if ( !mxIsEmpty(arr) )
        {
            copyMArrayToC_FORCESNLPsolver_normal(mxGetPr(arr), p, dimp);
        }
	}   
	...
	FORCESNLPsolver_normal_adtool2forces(z, y, l, p, obj, jacobj, c, jacc, h, jach, hess, stage, 0, 0);
	obj_mex = mxCreateDoubleMatrix(1, 1, mxREAL);
	gradobj_mex = mxCreateDoubleMatrix(nvar, 1, mxREAL);
	copyCArrayToM_FORCESNLPsolver_normal(obj, mxGetPr(obj_mex), 1);
	copyCArrayToM_FORCESNLPsolver_normal(jacobj, mxGetPr(gradobj_mex), nvar);
	plhs[0] = obj_mex;
	plhs[1] = gradobj_mex;
